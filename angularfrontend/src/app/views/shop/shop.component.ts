...
export class ShopComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log('Zalogowany jako ' + this.userData.username + ' ID: ' + this.userData.id);
    this.addShopItems();
    this.listUserItems();
  }
  ...
  addShopItems() {
    ...
    // TODO: tutaj id
    this.dropService.getShopItemsForUser(this.userData.id).subscribe(response => {
  ...
  listUserItems() {
    this.userItemsService.getUserItems(this.userData.id).subscribe(response => { // W PRZYSZLOSCI NIE MOZNA POBIERAC LOSOWYCH OFC
  ...
  drop(event: any) {
    ...
    if (this.previusDragContainer.includes('shop') && event.container.element.nativeElement.id.includes('slot')) {
      if (event.container.element.nativeElement.children[0].children.length === 0) {
        ...
        // TODO: tutaj id
        this.userItemsService.addItemToUser(this.userData.id, event.item.element.nativeElement.children[0].id,
    ...
    if (event.container.element.nativeElement.id.includes('shop') && this.previusDragContainer.includes('slot')) {
      // TODO: tutaj id
      this.userItemsService.removeItemFromUser(this.userData.id, 'userItem' + event.previousContainer.element.nativeElement.id.split('-')[1])
        .subscribe();
